...
const shouldShowIntro = () => {
  ...
  if (ref) {
    try {
      ...
      if (refUrl.hostname === window.location.hostname && window.sessionStorage.getItem('fitbuddy-skip-intro') === '1') {
...
const IntroBubbles: React.FC<{ onFinish: () => void }> = ({ onFinish }) => {
  ...
  useEffect(() => {
    ...
    timers.push(setTimeout(() => {
      setHide(true);
      console.log('[IntroBubbles] Hide set to true.');
    }, totalFlyIn + 4500));
    // Call onFinish as soon as fadeout starts, so the home page fades in right after bubbles disappear
    timers.push(setTimeout(() => {
      console.log('[IntroBubbles] onFinish called.');
      onFinishRef.current();
    }, totalFlyIn));
    return () => {
