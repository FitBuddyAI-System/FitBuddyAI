...
const modules = import.meta.glob('../data/exercises/*.json', { eager: true }) as Record<string, { default: Partial<Workout> } | Partial<Workout>>;
...
const buildWorkoutLibrary = () => {
  ...
  const items = Object.entries(modules).map(([path, mod]) => {
    const data = ((mod && 'default' in mod) ? mod.default : mod) as Partial<Workout>;
